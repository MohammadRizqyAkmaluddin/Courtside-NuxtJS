...
<template>
    <div class="font-inter navbar fixed top-0 left-0 w-full z-20 bg-white flex items-center justify-between px-[40px] min-[1000px]:px-[100px] min-[1000px]:py-2 py-3 border-b">
        <!-- Logo -->
    ...
    <div :class="[ 'fixed top-0 right-0 h-full w-[70%] max-w-[320px] bg-white z-50 shadow-xl transform transition-transform duration-300',
        ...
        <div class="flex flex-col justify-between min-h-full pb-10">
            <div class="flex flex-col gap-10">
                <div v-if="!authUser.user" class="flex items-center justify-between gap-3 text-center">
                    <button @click="openLogin" class="text-gray-800 text-lg">Login</button>
                    <button @click="openRegister" to="/auth/register/user" class="flex items-center justify-center gap-2 font-bold border border-blue-900 px-6 py-2 rounded-full text-blue-900 hover:bg-gray-100">Register</button>
                </div>
                <div v-else class="flex gap-5 ps-5 pt-10 items-center justify-start border-b pb-10">
                ...
                <div class="flex flex-col text-sm text-gray-800 font-semibold">
                    <NuxtLink @click="closeMenu" to="/court" :class="isCourtActive ? 'bg-gray-100' : ''" class="p-4 px-5 border-b flex items-center gap-2">
                        <Icon icon="ph:court-basketball-light" width="20" height="20" /> Explore Venue
                    </NuxtLink>
                    <NuxtLink @click="closeMenu" to="/community" :class="isCommunityActive ? 'bg-gray-100' : ''" class="p-4 px-5 border-b flex items-center gap-2">
                        <Icon icon="fluent:people-audience-32-regular" width="20" height="20" />Sport Community
                    </NuxtLink>
                    <NuxtLink @click="closeMenu" to="/sparring" :class="isSparringActive ? 'bg-gray-100' : ''" class="p-4 px-5 border-b flex items-center gap-2">
                        <Icon icon="icon-park-outline:people-speak" width="20" height="17" />Find Sparring
                    </NuxtLink>
                    <NuxtLink @click="closeMenu" to="/cup" :class="isLeagueActive ? 'bg-gray-100' : ''" class="p-4 px-5 border-b flex items-center gap-2">
                        <Icon icon="icons8:trophy" width="20" height="20" />Courtside Cup
                    </NuxtLink>
                    <NuxtLink v-if="authUser.user" @click="closeMenu" to="/user/activity" :class="isActivityActive ? 'bg-gray-100' : ''" class="p-4 px-5 flex items-center gap-2">
                        <Icon icon="icon-park-outline:activity-source" width="20" height="17" />My Activity
                    </NuxtLink>
                </div>
            </div>
            <button @click="logout" class="text-sm flex items-center justify-center gap-1 font-bold border border-blue-900 px-4 py-1 mx-5 rounded-full text-blue-900 hover:bg-gray-100">
